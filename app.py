...
# Removed unused imports: google.generativeai, langchain_tavily
import chromadb
from sentence_transformers import SentenceTransformer
from langchain_text_splitters import RecursiveCharacterTextSplitter
# Changed to import specific components from LangChain
...
def retrieve_documents(query: str) -> str:
    ...
    try:
        ...
        # Reduced n_results to 3 to optimize prompt size/latency
...
def create_agent():
    ...
    # Using the fast, robust Gemini model name
    gemini_model_name = st.secrets.get("GEMINI_MODEL_NAME", "gemini-2.5-flash") 

    llm = ChatGoogleGenerativeAI(
        google_api_key=gemini_api_key,
        model=gemini_model_name,
        temperature=0.1 # Low temperature for more reliable agent behavior
    )
    
    agent = create_react_agent(llm, tools, prompt_template)
    
    # Set handle_parsing_errors=True for robust chat interaction
...
def handle_user_input():
    """Handles user input and gets a streamed response from the agent."""
    if prompt := st.chat_input("Ask about your document or a general question..."):
        ...
        with st.chat_message("assistant"):
            # Prepare the placeholder for the streaming response
            response_placeholder = st.empty()
            full_response = ""
            
            agent_executor = create_agent()
            
            # Format and truncate chat history for the agent to maintain context
            # Convert session messages to LangChain message objects
            history_for_agent = []
            # Only send the last 10 messages (5 turns) to prevent prompt bloat
            ...
            try:
                # Use .stream() for low latency appearance
                for chunk in agent_executor.stream({
                    "input": prompt, 
                    "chat_history": history_for_agent
                }):
                    # LangChain AgentExecutor stream provides chunks with an 'output' key
                    if chunk.get("output"):
...
with st.sidebar:
    ...
    if 'chat_history' in st.session_state and st.session_state.chat_history:
        ...
        for chat_id in sorted_chat_ids:
            ...
            # Using HTML/Markdown for styling the button text
